...
class IntegrationBase
{
  public:
    ...
    // 这里的初始化参数分别是：上一帧的加速度，上一帧的角速度，上一帧的加速度的bias，上一帧的角速度的bias，
    // 时间间隔，15维度的全雅可比矩阵，协方差矩阵，预积分量 alpha gama beta
    {
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        Vector3d un_acc_0 = delta_q * (_acc_0 - linearized_ba);  /// 上一帧的加速度在初始帧坐标系下的值
        Vector3d un_gyr = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg;  // 角速度的中值计算，IMU帧间计算
        ...
        // Step 2 随后更新方差矩阵及雅克比
        // 也就是预积分IMU帧间的离散化递推公式F矩阵以及对应的噪声矩阵V（笔记公式是G）求解和雅可比矩阵以及协方差矩阵的更新
        if(update_jacobian)
        {
    ...
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        // 入参：时间间隔，上一帧的加速度，上一帧的角速度，当前帧的加速度，当前帧的角速度，
        // 上一帧预积分量 alpha gama beta，上一帧的加速度的bias，上一帧的角速度的bias，
        // 出参：当前帧预积分量 alpha gama beta，当前帧的加速度的bias，当前帧的角速度的bias
